...
try:
    ...
    logger.info("flash_attn is installed. Flash attention will be available.")
except ImportError:
    IS_FLASH_ATTN_AVAILABLE = False
    logger.warning("flash_attn is not installed. Flash attention will not be available.")
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg):
        ...
        # pytorch module
        if cfg["use_flash_attention"] and IS_FLASH_ATTN_AVAILABLE:
            logger.info("Using Flash Attention in TransformerBlock.")
            self.att = MHA(
                embed_dim=cfg["emb_dim"],
                num_heads=cfg["n_heads"],
                dropout=cfg["drop_rate"],
                use_flash_attn=True,
                # by default, no attention window is used, passing (-1, -1) to attention_window_size
                window_size=cfg["attention_window_size"],
                causal=True,
            )
        else:
            logger.info("Using standard MultiheadAttention in TransformerBlock.")
    ...
    def forward(self, x):
        ...
        if self.use_flash_attention and IS_FLASH_ATTN_AVAILABLE:
            # MHA from flash_attn
            x = self.att(x)
...
class GPTModel(nn.Module):
    ...
    def forward(self, in_idx, memory_tracker: MemoryTracker | None = None):
        if memory_tracker is not None:
            memory_tracker.start_forward()
...
class CustomDecoderClassifier(nn.Module):
    def __init__(self, checkpoint, num_labels, unfreeze_last_n_layers=3):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size, 512), nn.LayerNorm(512), nn.GELU(), nn.Dropout(0.4), nn.Linear(512, num_labels)
        )
