...
def get_model_config(
    model_type: Literal["rnn", "transformer"],
    use_flash_attention: bool = False,
    vocab_size: int = 50257,
    attention_window_size: Tuple[int, int] = (-1, -1),
):
    if model_type == "rnn":
...
def calc_loss_batch(
    input_batch, target_batch, model, device, tokenizer, calculate_perplexity=False, memory_tracker: MemoryTracker | None = None
):
    input_batch, target_batch = input_batch.to(device), target_batch.to(device)
    logits = model(input_batch, memory_tracker=memory_tracker)

    logits_flat = logits.flatten(0, 1)
    targets_flat = target_batch.flatten()
    loss = F.cross_entropy(logits_flat, targets_flat, ignore_index=tokenizer.pad_token_id)
    perplexity = torch.exp(torch.tensor(loss.item())) if calculate_perplexity else None
...
def calc_loss_loader(data_loader, model, device, tokenizer, num_batches=None):
    ...
    for i, (input_batch, target_batch) in enumerate(data_loader):
        if i < num_batches:
            loss, perplexity = calc_loss_batch(input_batch, target_batch, model, device, tokenizer, calculate_perplexity=True)
    ...
    return (total_loss / num_batches, total_perplexity / num_batches)
...
if __name__ == "__main__":
    ...
    extract_speakleash_data_to_files(
        dataset_name="plwiki",
        output_dir="train_datasets/plwiki",
        max_docs=100000,
