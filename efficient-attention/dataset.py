...
class SpeakleashDataLoader(TorchDataset):
    def __init__(
        self,
        tokenizer,
        max_length,
...
def create_huggingface_dataloader(
    ...
    dataset_tokenized[split] = dataset_tokenized[split].map(
        tokenize, batched=True, fn_kwargs={"tokenizer": tokenizer, "max_length": max_length}
    )
    dataset_tokenized[split].set_format(type="torch", columns=["input_ids", "attention_mask", "labels"])

    dataloader = DataLoader(dataset_tokenized[split], batch_size=batch_size, shuffle=shuffle, pin_memory=True)
...
if __name__ == "__main__":
    ...
    loader = create_speakleash_dataloader(
        split="test", batch_size=32, speakleash_dataset_name="plwiki", tokenizer=tokenizer, drop_last=True, stride=256, max_length=256
    )
    iter = 0
    print(len(loader))
    print(loader)
    ...
    print(f"Input IDs list length: {len(loader.dataset.input_ids)}")
    print(f"Target IDs list length: {len(loader.dataset.target_ids)}")
